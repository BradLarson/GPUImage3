...
class FilterDisplayViewController: UIViewController, UISplitViewControllerDelegate {
    ...
    func configureView() {
        guard let videoCamera = videoCamera else {
            let errorAlertController = UIAlertController(
                ...
                message: "Couldn't initialize camera", preferredStyle: .alert)
        ...
        if let currentFilterConfiguration = self.filterOperation {
            ...
            if let view = self.filterView {
                ...
                case let .custom(filterSetupFunction: setupFunction):
                    currentFilterConfiguration.configureCustomFilter(
                        setupFunction(videoCamera, currentFilterConfiguration.filter, view))
                }
    ...
    @IBAction func updateSliderValue() {
        if let currentFilterConfiguration = self.filterOperation {
            switch currentFilterConfiguration.sliderConfiguration {
            case .enabled(_, _, _):
                currentFilterConfiguration.updateBasedOnSliderValue(Float(self.filterSlider!.value))
            case .disabled: break
            }
        }
    }
